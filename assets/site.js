...
async function renderList(el, { index = "posts/posts.json", base = "/blog/", limit } = {}) {
...
async function renderArticle({ index, dir, backHref }) {
  /* slug comes from ?p= on the legacy pages (post.html / project.html) or from
     the path on the pretty URLs generated by scripts/build-pages.js (/blog/<slug>/) */
  let slug = new URLSearchParams(location.search).get("p");
  if (!slug) {
    const parts = location.pathname.split("/").filter(Boolean);
    if (parts.length > 1) slug = decodeURIComponent(parts[parts.length - 1]);
  }
